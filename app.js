...
// ===== 因子マッピング =====
// 6因子＝インプット指標（先行指標 / 人的資本KPIに直結）
const FACTORS = {
  A: { label: '業務負荷',             questions: ['Q05','Q06','Q14'] },
  B: { label: '業務適応',             questions: ['Q01','Q02','Q03','Q17'] },
  C: { label: '関係性の共有度',       questions: ['Q08','Q11','Q12'] },
  D: { label: '関係性の心理的安全性', questions: ['Q07','Q09','Q10'] },
  E: { label: '自己調整感',           questions: ['Q04','Q13','Q15','Q16','Q18'] },
  F: { label: '行動の変化',           questions: ['Q19','Q20','Q21','Q22','Q23'] },
};
// アウトカム指標（結果指標 / 6因子の効果を測る）
const OUTCOME = {
  G: { label: '継続定着意思',         questions: ['Q24','Q25','Q26','Q27','Q28'] },
};
...
# 分析ロジック（出力には含めないこと）
1. 自己調整感が低い：30日目に「エネルギー枯渇による離脱」を予測。
2. 業務適応が低い：60日目に「役割へのミスマッチ」を予測。
3. 行動の変化が出ている、または継続定着意思が低い：90日目に「心理的撤退の完了」を予測。
...
【インプット指標：6因子】
・業務負荷：[Score_A]（※1に近いほど負荷が高い／高すぎる）
・業務適応：[Score_B]
・関係性の共有度：[Score_C]
・関係性の心理的安全性：[Score_D]
・自己調整感：[Score_E]
・行動の変化：[Score_F]（※1に近いほど、無意識のミスや反応の遅れが出やすい状態）
...
関係性の共有度・心理的安全性と継続定着意思の相関を分析してください。
...
【インプット指標：6因子】
・業務負荷：[Score_A] / 5.0（※低いほど負荷が高い）
・業務適応：[Score_B] / 5.0
・関係性の共有度：[Score_C] / 5.0
・関係性の心理的安全性：[Score_D] / 5.0
・自己調整感：[Score_E] / 5.0
・行動の変化：[Score_F] / 5.0（※低いほど離脱行動のサインが強い）
...
function calcScores(answers) {
  const scores = {};
  const all = { ...FACTORS, ...OUTCOME };
  for (const [key, factor] of Object.entries(all)) {
...
function renderScoreBars(scores) {
  ...
  const renderGroup = (title, entries) => {
    container.innerHTML += `<div class="score-section-label">${title}</div>`;
  ...
  renderGroup('📊 インプット指標（6因子・先行指標）', Object.entries(FACTORS));
  renderGroup('🎯 アウトカム指標（結果指標）', Object.entries(OUTCOME));
}
...
function renderRadarChart(scores) {
  ...
  const vals = Object.keys(FACTORS).map(k => scores[k] / 5);
...
function getGroupFromQuestion(qNum) {
  if (qNum >= 1  && qNum <= 6)  return 'work';     // 業務負荷・業務適応
  if (qNum >= 7  && qNum <= 12) return 'relation'; // 関係性の共有度・心理的安全性
  if (qNum >= 13 && qNum <= 18) return 'self';     // 自己調整感
  if (qNum >= 19 && qNum <= 23) return 'behavior'; // 行動の変化
  if (qNum >= 24 && qNum <= 28) return 'outcome';  // 継続定着意思
  return 'general';
}
